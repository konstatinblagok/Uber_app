...
class PayPalController extends Controller
{
    ...
    public function pay(Request $request) {
        ...
        try {
            ...
            if(strtolower($mealData->foodType->name) == 'dessert' || strtolower($mealData->foodType->name) == 'desert') {

                if(Session::get('globalMealPrice') >= 12 && Session::get('globalMealQuantity') > 1) {

                    $totalFreeDeliveryQuantityTemp = floor(Session::get('globalMealPrice') / 12);

                    if($totalFreeDeliveryQuantityTemp >= Session::get('globalMealQuantity')) {

                        $totalFreeDeliveryQuantityTemp = $totalFreeDeliveryQuantityTemp - 1;
                    }

                    $deductedPrice = $totalFreeDeliveryQuantityTemp * getDeliveryChargesAmount();

                    Session::put('globalMealDeliveryPrice', (Session::get('globalMealDeliveryPrice') - $deductedPrice) > getDeliveryChargesAmount() ? Session::get('globalMealDeliveryPrice') - $deductedPrice : getDeliveryChargesAmount());
                }
            }
            ...
            $item_2->setName('Delivery Charges (Based on No. of Portions)')
                ->setCurrency('EUR')
                ->setQuantity((Session::get('globalMealDeliveryPrice')/getDeliveryChargesAmount()))
                ->setPrice(getDeliveryChargesAmount());
        ...
        try {
            ...
            if ($result->getState() == 'approved') {
                ...
                if(isset($billingInfoID)) {
                    ...
                    //Send Confirmation Email to Customer & Cook
                    sendOrderConfirmationEmail(Auth::user()->email, $order->id);
                    sendOrderConfirmationEmail($userData->email, $order->id);
                }
