                                    ...
                                    @if(count($orderList) > 0)
                                        @foreach ($orderList as $order)
                                            <tr>
                                                ...
                                                <td>
                                                    <a class="btn btn-secondary" href="{{ route('cook.order.detail', ['id' => $order->id]) }}" title="View Details">Details</a>

                                                    @if($order->review == null && $order->status == 'Delivered')
                                                        <a class="btn btn-chezdon" href="{{ route('cook.order.review', ['id' => $order->id]) }}" title="Review">Give Review</a>
                                                    @endif
                                                </td>    
                                            </tr>
                                        @endforeach
