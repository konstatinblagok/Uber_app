                                    ...
                                    @if(count($orderList) > 0)
                                        @foreach ($orderList as $order)
                                            <tr>
                                                ...
                                                <td>
                                                    <a class="btn btn-secondary" href="{{ route('customer.order.detail', ['id' => $order->id]) }}" title="View Details">@lang('lang.Details')</a>

                                                    @if($order->review == null && $order->status == 'Delivered')
                                                        <a class="btn btn-chezdon" href="{{ route('customer.order.review', ['id' => $order->id]) }}" title="Review">@lang('lang.Give Review')</a>
                                                    @endif
                                                </td>    
                                            </tr>
                                        @endforeach
