        ...
        if($order) {
            ...
            if($order->user_id == Auth::user()->id) {
                ...
                if($order->status == 'Delivered') {
                    ...
                    return view('customer.order.review', $data);
                }
                else {
    
                    return redirect()->route('customer.order.history')->with('error', 'You can only give review when the order has been delivered!');
                }
            }
            else {

                return redirect()->route('customer.order.history')->with('error', 'You have not authorized for this order!');
            }
        }
        else {

            return redirect()->route('customer.order.history')->with('error', 'Order not found!');
        }
    }
